...
func TestPostgresBlockPersistenceRollsBackOrphanedBlocksAtomically(t *testing.T) {
	...
	parentHash := BlockHash("0xreorg-parent")
	for blockNumber := firstBlock; blockNumber <= lastBlock; blockNumber++ {
		blockHash := BlockHash(fmt.Sprintf("0xreorg-%d", blockNumber))
	...
	ancestor := BlockCheckpoint{
		Number: firstBlock,
		Hash:   BlockHash(fmt.Sprintf("0xreorg-%d", firstBlock)),
	}
