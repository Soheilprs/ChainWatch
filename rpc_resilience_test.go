...
func TestExecuteResilientRPCExhaustsBoundedRetryBudget(t *testing.T) {
	...
	if !errors.Is(err, ErrTemporaryDependency) || !errors.Is(err, cause) {
...
func TestRetryableRPCErrorClassification(t *testing.T) {
	tests := []struct {
		...
		{name: "validation", err: NewDomainError(ErrValidation, "validate", errors.New("bad input")), retryable: false},
	}
...
func newTestResilientClient(metrics *Metrics, maxAttempts int) *ResilientEthereumClient {
