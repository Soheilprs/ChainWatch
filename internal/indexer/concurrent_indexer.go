...
func (c *ConcurrentRangeIndexer) IndexRange(
	...
	defer domain.ClassifyError(&err, domain.ErrIndexing, "index block range concurrently")
	...
	if c.workerCount <= 0 {
		return nil, domain.NewDomainError(domain.ErrValidation, "validate worker count", ErrInvalidWorkerCount)
	}
