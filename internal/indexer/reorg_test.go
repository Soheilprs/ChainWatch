...
func TestReorgManagerRollsBackToCommonAncestor(t *testing.T) {
	...
	for _, index := range []domain.BlockTransferIndex{
	...
	client := &MockTransferBlockClient{Blocks: map[uint64]domain.ObservedBlock{
...
func TestReorgManagerRejectsReorgBeyondMaximumDepth(t *testing.T) {
	...
	if !errors.Is(err, ErrReorgTooDeep) || !errors.Is(err, domain.ErrChainReorg) {
...
func TestReorgManagerFailsSafeWithoutHistory(t *testing.T) {
	store := NewMemoryCheckpointStore()
	ctx := context.Background()
	if err := store.Save(ctx, domain.BlockCheckpoint{Number: 102, Hash: "0x102a"}); err != nil {
	...
	if !errors.Is(err, ErrReorgHistoryUnavailable) || !errors.Is(err, domain.ErrChainReorg) {
...
func TestReorgManagerRecordsCanonicalLegacyCheckpoint(t *testing.T) {
	store := NewMemoryCheckpointStore()
	ctx := context.Background()
	checkpoint := domain.BlockCheckpoint{Number: 102, Hash: "0x102a"}
