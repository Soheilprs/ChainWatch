...
func TestHTTPServerListTransfers(
	...
	reader :=
		&MockTransferReader{
			Page: domain.TransferPage{
				Transfers: []domain.StoredERC20Transfer{
					{
						BlockNumber: 100,
...
func TestHTTPServerMapsDomainErrors(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "not found",
			err:        domain.NewDomainError(domain.ErrNotFound, "load resource", errors.New("private lookup details")),
			wantStatus: http.StatusNotFound,
			wantBody:   "resource not found",
		},
		{
			name:       "temporary dependency",
			err:        domain.NewDomainError(domain.ErrTemporaryDependency, "query dependency", errors.New("private network details")),
