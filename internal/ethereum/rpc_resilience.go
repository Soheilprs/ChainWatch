...
type resilientEthereumBackend interface {
	GetLatestObservedBlock(context.Context) (domain.ObservedBlock, error)
	GetObservedBlockByNumber(context.Context, uint64) (domain.ObservedBlock, error)
	GetERC20TransfersByBlock(context.Context, domain.ObservedBlock) (domain.BlockTransferIndex, error)
	FetchTokenMetadata(context.Context, domain.Address) (domain.TokenMetadata, error)
}
...
func NewResilientEthereumClient(
	...
	if err := config.Validate(); err != nil {
		return nil, domain.NewDomainError(domain.ErrValidation, "validate RPC resilience configuration", err)
	}
	if metrics == nil {
...
func (c *ResilientEthereumClient) GetLatestObservedBlock(
	...
	return executeResilientRPC(ctx, c, func(ctx context.Context) (domain.ObservedBlock, error) {
...
func (c *ResilientEthereumClient) GetERC20TransfersByBlock(
	...
	return executeResilientRPC(ctx, c, func(ctx context.Context) (domain.BlockTransferIndex, error) {
...
func (c *ResilientEthereumClient) FetchTokenMetadata(
	...
	return executeResilientRPC(ctx, c, func(ctx context.Context) (domain.TokenMetadata, error) {
...
func executeResilientRPC[T any](
	...
	for attempt := 1; attempt <= client.policy.maxAttempts; attempt++ {
		...
		if attempt == client.policy.maxAttempts {
			client.metrics.RecordRPCFailure()
			return zero, domain.NewDomainError(
				domain.ErrTemporaryDependency,
...
func isRetryableRPCError(err error) bool {
	if err == nil ||
		...
		errors.Is(err, domain.ErrInvalidTokenAddress) ||
		errors.Is(err, domain.ErrChainReorg) {
		return false
	}
	if errors.Is(err, domain.ErrTemporaryDependency) ||
