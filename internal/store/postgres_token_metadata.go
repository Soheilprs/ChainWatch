...
) (metadata domain.TokenMetadata, exists bool, err error) {
	defer domain.ClassifyError(&err, domain.ErrDatabase, "load token metadata")
...
func (s *PostgresTokenMetadataStore) SaveTokenMetadata(
	...
	defer domain.ClassifyError(&err, domain.ErrDatabase, "save token metadata")
