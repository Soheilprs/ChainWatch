...
func (s *PostgresBlockPersistence) SaveIndexedBlock(
	...
	defer domain.ClassifyError(&err, domain.ErrDatabase, "atomically persist indexed block")
...
func (s *PostgresBlockPersistence) EnsureIndexedBlock(
	...
	defer domain.ClassifyError(&err, domain.ErrDatabase, "record checkpoint block history")
...
func (s *PostgresBlockPersistence) LoadIndexedBlockHash(
	ctx context.Context,
	blockNumber uint64,
) (hash domain.BlockHash, exists bool, err error) {
	defer domain.ClassifyError(&err, domain.ErrDatabase, "load indexed block hash")
	...
	return domain.BlockHash(storedHash), true, nil
}
...
func (s *PostgresBlockPersistence) RollbackTo(
	...
	defer domain.ClassifyError(&err, domain.ErrDatabase, "roll back orphaned indexed blocks")
