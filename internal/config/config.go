...
// RPCConfig contains retry and rate-limit settings without coupling
// configuration loading to the concrete Ethereum adapter.
...
func LoadConfig() (Config, error) {
	...
	if err != nil {
		return Config{}, domain.NewDomainError(
			domain.ErrConfiguration,
...
func loadConfig(lookup environmentLookup) (Config, error) {
	...
	if value, exists := optionalEnvironment(lookup, "WORKER_COUNT"); exists {
		...
		if err != nil {
			return Config{}, domain.NewDomainError(domain.ErrValidation, "parse WORKER_COUNT", err)
		}
	}
...
func (c Config) Validate() error {
	...
	case strings.TrimSpace(c.EthereumRPCURL) == "":
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("ETH_RPC_URL is required"))
	case strings.TrimSpace(c.DatabaseURL) == "":
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("DATABASE_URL is required"))
	case strings.TrimSpace(c.HTTPAddress) == "":
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("HTTP_ADDRESS must not be empty"))
	case c.WorkerCount <= 0:
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("WORKER_COUNT must be greater than zero"))
	case c.PollInterval <= 0:
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("POLL_INTERVAL must be greater than zero"))
	case c.ShutdownTimeout <= 0:
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("SHUTDOWN_TIMEOUT must be greater than zero"))
	case c.ReadHeaderTimeout <= 0:
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("READ_HEADER_TIMEOUT must be greater than zero"))
	case c.ReadTimeout <= 0:
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("READ_TIMEOUT must be greater than zero"))
	case c.WriteTimeout <= 0:
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("WRITE_TIMEOUT must be greater than zero"))
	case c.IdleTimeout <= 0:
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("IDLE_TIMEOUT must be greater than zero"))
	case c.MaxReorgDepth == 0:
		return domain.NewDomainError(domain.ErrValidation, "validate configuration", errors.New("MAX_REORG_DEPTH must be greater than zero"))
	default:
		if c.ProfilingAddress != "" {
			if err := validateProfilingAddress(c.ProfilingAddress); err != nil {
				return domain.NewDomainError(domain.ErrValidation, "validate PPROF_ADDRESS", err)
			}
		}
		if err := c.RPCConfig().Validate(); err != nil {
...
func (c Config) RPCConfig() RPCConfig {
	return RPCConfig{
...
func intEnvironment(
	...
	if err != nil {
		return 0, domain.NewDomainError(domain.ErrValidation, "parse "+name, err)
	}
	return parsed, nil
}
